...
用法: python tests/test_phase3a.py
注意: 本文件为脚本式验证, pytest 收集时不执行 (需 __main__ 触发)。
"""
import sys, os
...
"""
    ...
    print("=" * 60)
    # 标记合同为已复核 (check_reminders 仅对 reviewed=1 的合同触发提醒)
    conn.execute("UPDATE contracts SET reviewed=1 WHERE id=?", (cid,))
    conn.commit()
    # 手动插入一条已逾期的应收
